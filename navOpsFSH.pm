...
{
	my ($name, $comment) = @_;
	$name    = foldToAscii(flattenNewlines($name));
	$comment = foldToAscii(flattenNewlines($comment));
...
# The Leaflet map create/modify/delete FSH waypoint entries used to live here;
# they moved to navLeaflet.pm (Leaflet feature code does not belong in the
# navOps clipboard layer).  They still call the FSH spoke primitives in this
# file (_checkFSHNameConflict / _buildFSHWpRecord / _fshSetWpLatLon /
# _fshWPRoutes) and _newNavUUID -- which therefore have a navLeaflet caller as
# well as the clipboard machinery.
