	...
	elsif ($obj_type eq 'track')
	{
		...
		push @features, {
			...
			properties => {
				...
				depth_cm        => [ map { winTreeBase::_trackPointDepthCm($_) } @$pts ],
			},
...
# The map-waypoint dialog (POST /waypoint/save) is handled pane-free in
# navLeaflet.pm now, so it works whether or not a Database window is open.
# navLeaflet does the db write via navDB and the map render via the pane-free
# _pushObjToLeaflet / _pullFromLeaflet helpers below (which ride %rendered_uuids
# without needing an instance), then refreshes any open Database panes.
...
sub _dbCommit
{
	...
	eval
	{
		for my $it (@$items)
		{
			...
			next if !%dirty;
			updateRecord($dbh, $table, $it->{uuid}, \%dirty);
