...
my $command_data = {
	...
	$WIN_E80					=> ['ESeries',				'Live ESeries contents'],
	...
	$COMMAND_REFRESH_E80_DB		=> ['Refresh ESeries Database',	'Re-query all waypoints, routes, groups, and tracks from the ESeries plotter'],
	$COMMAND_CLEAR_E80_DB		=> ['Clear',				'Delete all waypoints, routes, groups, and tracks from the ESeries plotter'],
	$COMMAND_REFRESH_WIN_E80	=> ['Refresh Window',		'Reload ESeries window from in-memory data'],
	$COMMAND_SAVE_E80_CONFIG	=> ['Save Configuration...',	'Save the ESeries display configuration to a folder'],
	$COMMAND_RESTORE_E80_CONFIG	=> ['Restore Configuration...',	'Restore a saved ESeries display configuration from a folder'],
	$COMMAND_CLEAR_E80_CONFIG	=> ['Clear Configuration',	'Reset the ESeries display configuration to factory defaults'],
	$COMMAND_GRAB_E80_SCREEN	=> ['Grab Screen...',		'Capture the live ESeries screen to a PNG image'],
	$COMMAND_E80_TIMED_TRACKS	=> ['Timed Track Recording...',	'Turn the ESeries timed-track recording (date/time + depth per point) on or off'],
	$COMMAND_E80_ABOUT			=> ['About ESeries...',	'Show identity, firmware, and build details for ESeries units on the network'],
