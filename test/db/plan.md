...
Tests run in two sections per master_runbook's Test Organization Convention: positives first (`db.<N>`), guards second (`db.G<N>`).

### Positive Tests

| Test | What it verifies |
|------|------------------|
| db.1   | Position allocator: 32 PASTE_NEW_BEFORE bisections force AutoCompact (stress test, self-contained) |
| db.2   | COPY WP -> PASTE_NEW (fresh UUID in [DST]; source unchanged) |
| db.3   | CUT WP -> PASTE (UUID preserved; collection_uuid changes to [DST]) |
| db.4   | DELETE WP success (no route refs) |
| db.5   | DELETE GROUP -- dissolve (group shell deleted; members reparented to grandparent) |
| db.6   | DELETE GROUP+WPS -- success (no route refs) |
| db.8   | DELETE BRANCH -- recursive, safe (descendants gone) |
| db.9   | COPY BRANCH -> PASTE_NEW (fresh UUIDs throughout; source unchanged; tracks silently skipped) |
| db.10  | CUT BRANCH -> PASTE (branch moves as a unit; UUID and contents preserved) |
| db.11  | COPY ROUTE -> PASTE_NEW (fresh route UUID; route_waypoints reference same WP UUIDs as source) |
| db.12  | CUT ROUTE -> PASTE (UUID and route_waypoints preserved; collection_uuid changes) |
| db.13  | CUT TRACK -> PASTE (UUID and track_points preserved; collection_uuid changes) |
| db.14a | PASTE_NEW_BEFORE with collection-member anchor (insert at midpoint) |
| db.14b | PASTE_NEW_AFTER with collection-member anchor |
| db.15a | PASTE_NEW_BEFORE on route point (copy-splice; route count +1) |
| db.15b | PASTE_BEFORE on route point (cut-splice; count unchanged) |
| db.16a | PASTE_NEW_BEFORE with route-object anchor |
| db.16b | PASTE_NEW_AFTER with route-object anchor |
| db.17  | PASTE_NEW_BEFORE with group-object anchor |
| db.18  | PASTE_NEW_BEFORE with branch-object anchor |
| db.19a | PASTE_NEW_BEFORE with route clipboard + WP anchor |
...
Renamed from previous numbers; guard ordering follows the original execution sequence so state dependencies are preserved.  Old-number cross-reference is kept inline for log/code archaeology.
