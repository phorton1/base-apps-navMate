...
- **Wire encoding (R3).** navMate `/api/ocpn` encodes ASCII `\uXXXX` via `JSON::PP`; R3 is codepoint-equality. `[OCPN_CAFE]` (foreign-accented, non-ASCII name + comment) is the probe.
